...
if(isset($_GET['id']) && $_GET['id'] > 0){
    ...
    if($qry->num_rows > 0){
		...
		echo '<script>alert("car_type ID is not valid."); location.replace("./?page=car_types")</script>';
	}
}else{
	echo '<script>alert("car_type ID is Required."); location.replace("./?page=car_types")</script>';
}
?>
<style>
